...
type
  TfrmJogoVelha = class(TForm)
    ...
    procedure btnreiniciarplacarClick(Sender: TObject);
  private
...
procedure TfrmJogoVelha.btnreiniciarplacarClick(Sender: TObject);
begin
  contX := 0;
  contO := 0;
  contEmp := 0;
  lblO.Caption := IntToStr(contO);  // ou lblO.caption := '0';
  lblX.Caption := IntToStr(contX);
  lblEmp.Caption := IntToStr(contO);
  btnReiniciar.Click; // ou btnreiniciarclick(btnreiniciar);
